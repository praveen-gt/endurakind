...
export default function WhyChooseSection() {
  ...
  // Reduced parallax movement: 50 → 30 for less calculation overhead
  ...
  return (
    <section
      ...
      className="relative py-20 md:py-32 overflow-hidden bg-white"
    >
      {/* Wavy Top - pointer-events-none for performance */}
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {features.map((feature, index) => (
                <motion.div
                  ...
                  className="group relative bg-gradient-to-br from-gray-50 to-white p-6 rounded-2xl border border-gray-100 hover:border-teal-200 hover:shadow-xl transition-all duration-300 ease-out"
                >
                  <div
                    className={`w-14 h-14 bg-gradient-to-br ${feature.color} rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300 ease-out shadow-lg`}
                  >
                    <feature.icon className="w-7 h-7 text-white" />
                  </div>
                  <h3 className="text-xl font-bold mb-2 text-gray-900 group-hover:text-teal-600 transition-colors duration-300 ease-out">
                    {feature.title}
                  </h3>
                  <p className="text-gray-600 text-sm leading-relaxed">
                    {feature.description}
          ...
          >
            <div className="relative w-full max-w-[550px] mx-auto">
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-teal-100 via-amber-50 to-white rounded-3xl blur-3xl opacity-60 pointer-events-none" />
              ...
              <div className="grid grid-cols-3 gap-3 relative z-10">
                ...
                <div className="col-span-2 rounded-xl overflow-hidden shadow-lg h-40 transition-transform duration-300 ease-out hover:scale-105">
                  <Image
                    ...
                    loading="eager"
                    quality={75}
                    sizes="(max-width: 640px) 66vw, (max-width: 1024px) 50vw, 400px"
                  />
                </div>
              ...
              {/* Floating Circle Animation - will-change for optimization */}
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                className="absolute top-1/2 left-1/2 w-96 h-96 border-4 border-dashed border-teal-300 rounded-full -z-10 -translate-x-1/2 -translate-y-1/2 pointer-events-none will-change-transform"
              />
            </div>
          </motion.div>
        </div>
      ...
      {/* Wavy Bottom - pointer-events-none for performance */}
